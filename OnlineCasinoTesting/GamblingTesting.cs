...
namespace OnlineCasinoTesting
{
    public class GamblingTesting
    {
        ...
        public void playSlotTestDouble(double bet, string username, bool prizeModuleBool)
        {
            ...
            string userDirectory = "Users\\" + username + "\\" + DateTime.Now.ToString("yyMM");
            string fRDirectory = "FinancialReport\\" + DateTime.Now.ToString("yyMM");
            string userFileName = "Users\\" + username + "\\" + DateTime.Now.ToString("yyMM") + "\\" + DateTime.Now.ToString("yyyyMMdd") + ".json";
            string fRFileName = "FinancialReport\\" + DateTime.Now.ToString("yyMM") + "\\" + DateTime.Now.ToString("yyyyMMdd") + ".json";
            _mockFileHandling.SetupAllProperties();
            _mockFileHandling.Setup(t => t.directoryExists(userDirectory)).Returns(true);
            _mockFileHandling.Setup(t => t.createDirectory(userDirectory));
            _mockFileHandling.Setup(t => t.fileExists(userFileName)).Returns(true);
            _mockFileHandling.Setup(t => t.directoryExists(fRDirectory)).Returns(true);
            _mockFileHandling.Setup(t => t.createDirectory(fRDirectory));
            _mockFileHandling.Setup(t => t.fileExists(fRFileName)).Returns(true);
            ...
            _mockFileHandling.Setup(t => t.readAllText(userFileName)).Returns("[{\"betAmount\":2.0,\"payout\":4.0}]");
            _mockFileHandling.Setup(t => t.readAllText(fRFileName)).Returns("[{\"betAmount\":2.0,\"payout\":4.0}]");
            _mockFileHandling.Setup(t => t.writeAllText(userFileName, gamblerAmountList));
            _mockFileHandling.Setup(t => t.writeAllText(fRFileName, fRAmountlList));
        ...
        public void playSlotTestTriple(double bet, string username, bool prizeModuleBool)
        {
            ...
            string fRFileName = "FinancialReport\\" + DateTime.Now.ToString("yyMM") + "\\" + DateTime.Now.ToString("yyyyMMdd") + ".json";
            _mockFileHandling.SetupAllProperties();
            _mockFileHandling.Setup(t => t.directoryExists(userDirectory)).Returns(false);
            _mockFileHandling.Setup(t => t.createDirectory(userDirectory));
            _mockFileHandling.Setup(t => t.fileExists(userFileName)).Returns(false);
            _mockFileHandling.Setup(t => t.directoryExists(fRDirectory)).Returns(false);
            _mockFileHandling.Setup(t => t.createDirectory(fRDirectory));
            _mockFileHandling.Setup(t => t.fileExists(fRFileName)).Returns(false);
